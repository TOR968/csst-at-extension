	...
	try {
		for (const t of await getProfileTargets(CDP)) {
			const res = await evalInTarget(CDP, t.targetId,
				"document.querySelector('.csstat-extension-container')?.getAttribute('data-csstat-injector') || ''",
				{ returnByValue: true });
			const val = res?.result?.value;
			if (val === 'webkit' || val === 'cdp') return val;
		}
	} catch (e) {
		console.error('[CSST.at] detect injector failed:', e);
	}
	return 'none';
}
...
async function setupCommunityInjection() {
	...
	const processTarget = (targetInfo: any) => {
		...
		pending.set(targetId, setTimeout(() => {
			...
			injectIntoTarget(targetId).catch((e: any) => console.error('[CSST.at] injection error:', e));
		}, 200));
	};
