...
**`frontend/inject.ts`** — Contains `csstatInjectMain(openExternal)`, a self-contained vanilla JS function that runs in the community browser via `Runtime.evaluate`. Serialized via `buildInjectionCode(openExternal)`, which stringifies the function and calls it with the flag: `` `(${csstatInjectMain.toString()})(${openExternal})` ``. Must have zero imports and no references to outer-scope variables — everything it needs is inside the function body (the `openExternal` flag is passed in as an argument).
